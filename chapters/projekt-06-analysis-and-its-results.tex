...
V rámci tohto skúmaného rozsahu som zistil, že HTTP Archive z nejakého dôvodu neobsahuje za mesiace máj a jún v roku 2022 žiadne NEL hlavičky žiadnom z uložených zdrojov. 
Taktiež som zistil, že pre január 2019 neexistujú žiadne HTTP Archive dáta.
To, že tieto dáta chýbajú sa vyzobrazilo na výsledných metrikách.
...
Neskôr som však zistil, že účty tejto služby sú viazané na debitné karty používateľa.
...
Z uvedených dát som získal výsledky celkovej práce použítím nástroja pre výpočet preddefinovaných metrík (viď sekciu \ref{analyze_results}).
...
\begin{itemize}
    \item počiatočný počet skúmaných domén -- 2 096 799, 
    \item počet skúmaných domén v poslednom skúmanom mesiaci -- 20 424 419,
    \item presný počet domén pri nárastoch a poklesoch v nasadení NEL:
    \begin{itemize}
...
Na tomto grafe, tak ako aj na niekoľkých ďalších, sa vyzobrazil výpadok dát za mesiace máj a jún v roku 2022.
...
Identifikoval som 425 kolektorov, ktoré sa za toto obdobie vyskytli.
Počet aktívnych kolektorov po jednotlivých mesiacoch uvádza graf v obrázku \ref{fig:httparchive-nel-collector-provider-top-1-over-time}.
...
\begin{center}
 ...
 \caption{\centering Graf zobrazujúci rast počtu poskytovateľov NEL kolektorov.}
...
Vizualizácia v obrázku \ref{fig:httparchive-nel-collector-provider-top-1-over-time} vyzobrazuje najpoužívanejšie NEL kolektory za skúmané obdobie. 
Vďaka tomuto grafu som zistil, že tie najviac badateľné nárasty v počte domén využívajúcich NEL sú spojené s objavením konkrétnych poskytovateľov NEL kolektorov:
\begin{itemize}
...
\begin{center}
 ...
 \caption{\centering Poskytovatelia NEL kolektorov s najvyšším počtom obsluhovaných domén pre každý skúmaný mesiac. }
 \label{fig:httparchive-nel-collector-provider-top-1-over-time}
\end{center}
\end{figure}
...
Počet ním osbsluhovaných domén totiž z júna 2022 na august 2022 narástol o 836 324.
...
Ďalej v obrázku \ref{fig:httparchive-nel-latest-collector-provider-stats} uvádzam graf vizualizujúci približné počty mesačne obsluhovaných domén tymito poskytovateľmi. Z grafu je možné vyčítať ako sa od prvého mesiaca funkčnosti daného poskytovateľa menil počet domén, ktoré obsluhuje.
Znova pripomínam, že v mesiacoch máj a jún 2022 sa nejedná o prerušenie dostupnosti jednotlivých poskytovateľov, ale o výpadok v HTTP Archive dátach.

\begin{figure}[!htb]
\begin{center}
 \includegraphics[scale=0.54]{obrazky-figures/httparchive_nel_latest_collector_provider_stats.pdf}
 \caption{\centering Vývin v počte obsluhovaných domén pre 10 najpoužívanejších poskytovateľov NEL kolektorov za apríl 2024.}
...
\begin{center}
 ...
 \caption{\centering Hodnoty konfiguračného poľa \code{include\_subdomains} počas skúmaného obdobia.}
...
\begin{center}
 ...
 \caption{\centering Hodnoty konfiguračného poľa \code{failure\_fraction} počas skúmaného obdobia.}
...
\begin{center}
 ...
 \caption{\centering Hodnoty konfiguračného poľa \code{success\_fraction} počas skúmaného obdobia.}
...
\begin{center}
 ...
 \caption{\centering Hodnoty konfiguračného poľa \code{max\_age} počas skúmaného obdobia.}
...
Podľa predošlých grafov je z tejto tabuľky možné vyčítať, že sa s jednoznačnou prevahou používajú v každom zvolenom mesiaci tie najpoužívanejšie hodnoty samostatných polí.
...
Vďaka tomu, že sú v dátach záznamy z každého zdroja dostupného na danej doméne, je možné tiež preskúmať, či doména využíva jednu, alebo viac variácií konfigurácie NEL.
...
Doplnkové dáta z vlastného skriptu na prehliadanie webu sa mi podarilo získať pre 32 921 domén z cieľových 58 596.
Dáta pre zvyšných 25 675 domén som získať nestihol.
...
Okrem toho som sa rozhodol použiť tu TRANCO aby som redukoval počet domén na prieskum, pričom ponechám v možine skúmaných tie relevantné.
...
Pre porovnanie s HTTP Archive dátami som teda z nich vybral domény aplikovaním rovnakých kritérií.

Z celkového počtu domén, 32 921, malo iba 27 785 nasadený NEL.
Nasadenie NEL je teda v tomto prípade 84.40\%.
Keďže tieto domény pochádzajú z HTTP Archive dát, predpokladom bolo, že všetky majú NEL nasadený.
Rozdiel je teda 
