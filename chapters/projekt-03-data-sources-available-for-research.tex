...
Niektoré sú vypracované na to aby spravovali napríklad zoznam domén, ktoré navštevujú používatelia služieb patriacich autorom daného projektu \cite{chrome-crux}. 
...
Selenium je dostupný pre viaceré programovacie jazyky, ako napríklad pre jazyk \textit{python}.
...
Funkcionalita pre čítanie HTTP prevádzky je ale v tomto prípade zabudovaná priamo do vlastného rozhrania Playwright \cite{crawling-playwright}.
...
Jednoznačnou prednosťou tejto metódy získavania vlastných dát pre analýzu je skutočnosť, že iným spôsobom nie je možné získať rozsiahle objemy dát popisujúce aktuálny stav nasadenia NEL.
...
\noindent Po pridaní tohto zdroja s priradeným názvom \code{httparchive} do projektu\footnote{\href{https://github.com/HTTPArchive/httparchive.org/blob/main/docs/gettingstarted\_bigquery.md}{https://github.com/HTTPArchive/httparchive.org/blob/main/docs/gettingstarted\_bigquery.md}} sa sprístupnia pre používanie datasety ako napríklad \cite{httparchive-repo}:
