...
\begin{enumerate}
    ...
    Príkladom potrebnej informácie pre HTTP žiadosť je hlavička \code{Host}, ktorej hodnota musí reprezentovať doménu cieľového web servera. \todo{nevedia co je domena}  
...
\begin{center}
    ...
    \caption{\centering Štruktúra HTTP žiadosti. Jednotlivé časti požiadavky sú na obrázku označené číslami. Postupne, prvá časť (1.) predstavuje metódu žiadosti, nasleduje cesta \mbox{k zdroju (2.)} --- v tomto prípade relatívna cesta, ktorú môže web server interpretovať podľa svojej konfigurácie. Ďalšia je vyznačená verzia protokolu HTTP (3.), pod ktorou je v poslednom rade umiestnená sekcia s HTTP hlavičkami (4.).}
...
asdjfkagfjkasdgfgjksaf

\begin{figure}[htb]
\begin{center}
    \includegraphics[scale=0.6]{obrazky-figures/http_response.png}
    \caption{\centering Štruktúra HTTP odpovede.}
