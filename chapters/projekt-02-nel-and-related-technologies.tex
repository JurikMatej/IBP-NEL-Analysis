...
Ak v pamäti webového prehliadača exisuje aspoň jedno pravidlo \code{NEL} využívajúce exitujúce pravidlo \code{Report-To}, webový prehliadač začne tvoriť a odosielať hlásenia typu \code{nel}\footnote{Adresa URL, na ktorú sú hlásenia odosielané definuje pravidlo \code{Report-To} (viď kapitolu \ref{reporting-api})}.
...
\begin{enumerate}
    ...
    Tieto skupiny definuje Reporting API svojimi pravidlami (pole \code{group} z HTTP hlavičky \code{Report-To} popísané v kapitole \ref{reporting-api}).
...
V tejto kapitole popisujem štruktúru a obsah týchto hlásení.
...
Polia \code{age}, \code{url} a \code{user\_agent} sú vyplnené bežne podľa štandardnej funkcionality Reporting API (viď kapitolu \ref{reporting-api}).
...
Štruktúra obsahu poľa \code{body} v hlásení typu NEL je premenná podľa toho, v ktorej časti operácia získavania monitorovaného zdroja zlyhala.
Pre zlyhania v rôznych častiach tejto operácie ďalej popisujem možné výsledné štruktúry tela hlásenia typu NEL.
...
\begin{itemize}
    ...
    Pre popis týchto dvoch polí viď predošlú kapitolu \ref{struktura-http-hlavicky-nel}, alebo výpis \ref{listing:priklad-nel-header}.
    ...
    % TODO prepisat celu tu hroznu strukturu volitelnych poli do nizzsie polozeneho bloku textu z rozdelenim podla faze
    \item \code{referrer}
    ...
    Ak však ide o hlásenie s fázou \code{phase} vyplnenej hodotou \code{"dns"} alebo \code{"connection"}, toto pole \textbf{nebude zahrnuté} do výsledného hlásenia.
        
    \item \code{server\_ip}

    Pole obsahuje IP adresu monitorovaného web servera, ktorá bola získaná DNS rezolúciou.
    Ak však ide o zlyhanie zaznamenané vo fáze (\code{phase}) \code{"dns"}, toto pole bude vyplnené ako \code{""} (stále bude zahrnuté do výsledného hlásenia).
    ...
    Ak je fáza zlyhania \code{"dns"}, toto pole bude mať hodnotu \code{""}.
    
    \item \code{method}

    Pole obsahuje HTTP metódu žiadosti, ktorá bola zaslaná klientom pre získanie monitorovaného zdroja.

    Toto pole je vyplnené iba v prípade, že fáza zlyhania je \code{"application"}, inak má hodnotu \code{""}.
    
    \item \code{request\_headers} a \code{response\_headers}

    Polia s obsahom popísaným v predošlej kapitole \ref{struktura-http-hlavicky-nel}.

    Ich vyplnenie je podmienené. Fáza zlyhania musí byť \code{"application"}, inak sú hodnoty polí vyplnené ako prázdne JSON polia (array).
    
    \item \code{status\_code}

    Pole s hodnotou HTTP statusu odpovede web servera vyplnené v prípade, že fáza je \code{"application"}, inak obsahuje prázdny reťazec \code{""}.
\end{itemize}

% TODO prepis
% V bode, keď je základná štruktúra pripravená sa mechanizmy tvorby hlásenia rozhodnú pridať ďalšie polia podľa obsahu už existujúceho poľa \code{phase}.
% Výsledné štruktúra môže byť jedna z nasledujúcich: 

% \begin{enumerate}
%     \item Hodnota poľa \code{phase} je \code{"dns"}:
%     \item Hodnota poľa \code{phase} je \code{"connection"}:
%     \item Hodnota poľa \code{phase} je \code{"}\code{application"}:
% \end{enumerate}


\begin{center}
\centering
\begin{lstlisting}[caption={\centering Príklad výsledného hlásenia typu NEL vytvoreného pravidlami popísanými vo výpise \ref{listing:priklad-nel-header}. Telo hlásenia bolo vygenerované pre zlyhanie vo fáze \code{"}\code{application"}. \mbox{Z webstránky} s adresou URL z poľa \code{referrer} bola zaslaná HTTP žiadosť typu \code{GET}, ktorú server obslúžil so statusom \code{404} -- zdroj sa nenašiel. Dodatočne, tak ako bolo definované v pravidlách NEL, telo obsahuje aj hlavičku \code{Content-Type}, ktorá však vyplnená nebola kvôli celkovému zlyhaniu navrátenia cieľového zdroja v odpovedi.},
...
% Informácia o fáze komunikácie popisujú v svojom poli \code{phase}, ktoré je popísané v predošlej kapitole \ref{struktura-hlasenia-nel}.
% Informáciu o type hl...
% V tejto kapitole vypisujem možné hodnoty pre 

% NEL rozdeľuje rôzne zlyhania, ktoré môžu nastať, do štyroch kategórií. 
% Prvé tri popisuje táto práca v kapitole \ref{body-zlyhania}.
